...
- `production` Environment 当前采用单人模式，不配置 required reviewer，避免触发者无法自审造成死锁；它只提供 `main` deployment branch policy 和生产 Secrets 作用域。生产发布仍由“先运行 `Production preflight`，再手动复制 `Release ID` 与 `Release SHA-256` 运行 `Deploy approved production`”完成。
...
受保护 PR 合并后，如果对应 `main` SHA 缺少任一可审计证据——真实 `PushEvent`、Actions run/check suite、CI 全部步骤成功或该 SHA 生成的正式 Artifact——立即阻断发布。旧 PR、旧 `main` SHA、旧 preflight 的 Artifact 一律禁止复用；不得用 `workflow_dispatch` 伪装 `push`，也不得手工重命名、复制或搬运旧 Artifact。

恢复必须新建一个无运行时影响的文档 PR，以产生新的 `main` SHA。该 PR 必须完整通过 PR CI，并经受保护合并后核验：真实 `main` push event、该新 SHA 的 CI 全部步骤成功，以及正式 Artifact 的不可变身份（至少绑定 repository、workflow、event=`push`、ref=`refs/heads/main`、commit SHA、run ID/attempt 与 Artifact ID/digest）。若连续再次漏发，停止发布并向 GitHub Support 提交 before/after SHA、PR、合并时间及缺失事件证据。生产 preflight 和 deploy 只能消费这个新 `main` push 生成并完成身份核验的正式 Artifact。
