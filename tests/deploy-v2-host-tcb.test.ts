...
describe("Host TCB v3 six-runtime exact-set",()=>{
  ...
  it("continuously audits sudoers and rejects any coexisting controller grant",async()=>{
    const control=await readFile("infra/deploy/peilv-control","utf8");expect(control).toContain("visudo -c");expect(control).toContain("/etc/sudoers.d -xdev -type f");expect(control).toContain("Conflicting peilv-control sudo authorization coexists");
  });
