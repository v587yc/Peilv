...
async function privilegedExists(root: string, target: string) {
  const checked = checkedFixturePath(root, target);
  if (isWindows) return lstat(checked).then(() => true, () => false);
...
async function fixture(options: FixtureOptions = {}) {
  ...
  if (!isWindows) { await exec("sudo", ["chown", "-R", "root:root", root]); for (const name of installNames) { await exec("sudo", ["chmod", modes[name].toString(8), path.join(stage, name)]); if (await lstat(destinations[name]).then(() => true, () => false)) await exec("sudo", ["chmod", modes[name].toString(8), destinations[name]]); } await exec("sudo", ["chmod", "0440", sudoersMain]); for (const dir of [stage, sbin, libexec, etc, sudoers]) await exec("sudo", ["chmod", "0755", dir]); await exec("sudo", ["chmod", "0700", state]); if (options.legacyOwner === "nobody") await exec("sudo", ["chown", "nobody:nogroup", destinations[legacyName]]); }
