...
async function fixture(failAfter=""){
  ...
  for(const name of objects){const target=path.join(stage,name);await writeFile(target,(await readFile(sources[name],"utf8")).replace(/\r\n/g,"\n"));await chmod(target,name==="peilv-sudoers"?0o440:name==="trusted-deploy-v2.sha256"?0o644:0o755);}
  ...
  const owner=process.platform==="win32"?"Administrator:UNKNOWN:1":`${process.env.USER}:${process.env.USER}:1`;
  script=script.replace('[[ "$(id -u)" == 0 ]]','[[ 0 == 0 ]]').replaceAll("root:root:1",owner).replace('"$(stat -c %a "$source")"','"${modes[$name]}"').replace("install -d -o root -g root -m 0755", "install -d -m 0755").replaceAll("install -o root -g root -m", "install -m");
  script=script.replace('flock -n 9 ||','true ||').replace('visudo -cf "$stage/peilv-sudoers"','true # visudo fixture');
