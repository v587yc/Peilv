...
export async function analyzeMatch(input: Record<string, unknown>): Promise<ServiceResult> {
...
export async function verifyBacktestPredictions(startDate: string, endDate: string): Promise<ServiceResult> {
  const params = new URLSearchParams({ startDate, endDate, source: "backtest" });
  const request = new NextRequest(`http://internal/api/analysis/verify?${params}`, {
    headers: internalHeaders(),
  });
  return unwrap(await verifyRoute(request));
}

export async function learnBacktestPatterns(runId: string, trainingWindowStart: string, trainingWindowEnd: string): Promise<ServiceResult> {
