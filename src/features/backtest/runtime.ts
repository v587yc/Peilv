...
const BACKTEST_LEASE_MS = 60_000;
const BACKTEST_HEARTBEAT_MS = 20_000;

export interface BacktestAnalysisServices {
  analyzeMatch(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  verifyBacktestPredictions(startDate: string, endDate: string): Promise<Record<string, unknown>>;
  learnBacktestPatterns(runId: string, trainingWindowStart: string, trainingWindowEnd: string): Promise<Record<string, unknown>>;
}
...
const leaseLostJobs = new Set<string>();

type BacktestClaimResult = { claimed?: boolean; reason?: string };

async function claimBacktestSlot(job: BacktestJob, maxConcurrentJobs: number, resume: boolean): Promise<BacktestClaimResult> {
  const { data, error } = await getSupabaseClient().rpc("claim_backtest_job", {
    ...
    p_max_concurrent: maxConcurrentJobs,
    p_resume: resume,
  });
  if (error) throw new Error(`回测并发配额领取失败: ${error.message}`);
  return (data && typeof data === "object" ? data : {}) as BacktestClaimResult;
}

async function persistJob(job: BacktestJob): Promise<void> {
  if (leaseLostJobs.has(job.id)) throw new Error("回测任务租约已失效，停止后续写入");
  ...
  const persisted = {
    ...
    lock_owner: job.status === "running" || job.status === "cancelling" ? job.lockOwner || null : null,
    lock_expires_at: job.status === "running" || job.status === "cancelling" ? job.lockExpiresAt || null : null,
    updated_at: new Date().toISOString(),
  };
  if (job.lockOwner) {
    const { data, error } = await supabase.rpc("persist_claimed_backtest_job", { p_job: persisted, p_lock_owner: job.lockOwner });
    if (error) throw new Error(`回测任务状态保存失败: ${error.message}`);
    if (data !== true) throw new Error("回测任务租约已失效，拒绝旧worker写入");
    return;
  }
  const { error } = await supabase.from("backtest_jobs").upsert(persisted, { onConflict: "id" });
  if (error) throw new Error(`回测任务状态保存失败: ${error.message}`);
}
...
export async function GET(request: NextRequest) {
  ...
  if ((persistedJob.status === "running" || persistedJob.status === "cancelling") && (!persistedJob.lockExpiresAt || new Date(persistedJob.lockExpiresAt) < new Date())) {
...
async function launchBacktestJob(
  ...
  dates: string[],
  timeoutMs: number,
  services: BacktestAnalysisServices,
): Promise<void> {
  jobs.set(job.id, job);
  const controller = new AbortController();
  leaseLostJobs.delete(job.id);
  jobControllers.set(job.id, controller);
  await persistJob(job);

  let heartbeatRunning = false;
  const heartbeat = setInterval(() => {
    if (!job.lockOwner || heartbeatRunning || controller.signal.aborted) return;
    heartbeatRunning = true;
    void Promise.resolve(getSupabaseClient().rpc("heartbeat_backtest_job", { p_job_id: job.id, p_lock_owner: job.lockOwner, p_lease_seconds: Math.ceil(BACKTEST_LEASE_MS / 1000) }))
      .then(({ data, error }) => {
        if (error || data !== true) {
          leaseLostJobs.add(job.id);
          controller.abort(new Error("回测任务租约已失效"));
          return;
        }
        job.lockExpiresAt = new Date(Date.now() + BACKTEST_LEASE_MS).toISOString();
      })
      .catch(() => {
        leaseLostJobs.add(job.id);
        controller.abort(new Error("回测任务租约心跳失败"));
      })
      .finally(() => { heartbeatRunning = false; });
  }, BACKTEST_HEARTBEAT_MS);
  ...
  const timeout = setTimeout(() => {
    ...
    if (!leaseLostJobs.has(job.id)) void persistJob(job).catch(() => undefined);
  }, timeoutMs);

  runBacktest(job, "", maxMatches, dates, controller.signal, services)
    .catch(async (error) => {
      ...
      job.endedAt = new Date().toISOString();
      if (!leaseLostJobs.has(job.id)) await persistJob(job).catch(() => undefined);
    })
    .finally(() => {
...
export async function POST(request: NextRequest, services: BacktestAnalysisServices) {
  ...
  if (resumeJobId) {
    ...
    job.lockOwner = randomUUID();
    job.lockExpiresAt = new Date(Date.now() + BACKTEST_LEASE_MS).toISOString();
    const claim = await claimBacktestSlot(job, limits.maxConcurrentJobs, true);
    if (!claim.claimed) {
      const status = claim.reason === "already_active" ? 409 : 429;
      return NextResponse.json({ error: claim.reason === "already_active" ? "任务仍在运行" : `running backtest limit reached (${limits.maxConcurrentJobs})` }, { status });
    }
    try { await launchBacktestJob(job, validated.maxMatches, remainingDates, limits.timeoutMs, services); }
    catch (error) { await failClaimedBacktest(job, error); throw error; }
  ...
  const job: BacktestJob = {
    ...
    lockExpiresAt: new Date(Date.now() + BACKTEST_LEASE_MS).toISOString(),
  };
  const claim = await claimBacktestSlot(job, limits.maxConcurrentJobs, false);
  if (!claim.claimed) {
    return NextResponse.json({ error: `running backtest limit reached (${limits.maxConcurrentJobs})` }, { status: 429 });
  }
  try { await launchBacktestJob(job, validated.maxMatches, validated.dates, limits.timeoutMs, services); }
  catch (error) { await failClaimedBacktest(job, error); throw error; }

  return NextResponse.json({ success: true, jobId, limits: { maxMatches: validated.maxMatches, timeoutMs: limits.timeoutMs } });
}

async function failClaimedBacktest(job: BacktestJob, error: unknown): Promise<void> {
  if (!job.lockOwner) return;
  await getSupabaseClient().rpc("fail_claimed_backtest_job", { p_job_id: job.id, p_lock_owner: job.lockOwner, p_safe_error: "回测任务启动失败" });
  jobs.delete(job.id);
  jobControllers.delete(job.id);
  console.error("[Backtest] Claimed job failed to launch:", error);
}

export async function reconcileExpiredBacktestLeases(limit = 25): Promise<number> {
  const { data, error } = await getSupabaseClient().rpc("reconcile_expired_backtest_jobs", { p_limit: Math.max(1, Math.min(limit, 100)) });
  if (error) throw new Error("回测租约回收失败");
...
async function runBacktest(
  ...
  dateLoop: for (const date of dates) {
    ...
    for (const match of matches) {
      ...
      try {
        ...
        const analysisData = await services.analyzeMatch(analysisReq as unknown as Record<string, unknown>);
  ...
  try {
    const verifyData = await services.verifyBacktestPredictions(job.startDate, job.endDate);
  ...
  job.log.push("开始机器学习...");
  try {
    const learnData = await services.learnBacktestPatterns(job.id, job.startDate, job.endDate);
